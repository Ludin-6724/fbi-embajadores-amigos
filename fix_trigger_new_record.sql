-- FIX: Evitar error de "record new has no field is_anonymous"
...
BEGIN
  ...
  -- Corrección: Usar row_to_json(NEW) para evitar que PostgreSQL lance error cuando una tabla
  -- (como post_reactions) no tiene la columna "is_anonymous".
  IF (TG_TABLE_NAME IN ('comments', 'posts') AND (row_to_json(NEW)->>'is_anonymous') = 'true') THEN
