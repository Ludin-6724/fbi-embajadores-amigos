...
BEGIN
    ...
    -- Si no hay actor o el actor falló, salir (safety check)
    ...
    -- Si la acción es anónima (para comentarios), cambiar nombre
    ...
    ELSIF (TG_TABLE_NAME = 'comments') THEN
        ...
        -- Notificar al creador de la publicación (si no es él mismo)
        ...
        -- Notificar al creador del comentario padre (si es una respuesta)
        IF (NEW.parent_id IS NOT NULL) THEN
            SELECT author_id INTO target_comment_author FROM public.comments WHERE id = NEW.parent_id;
            
            -- Para no notificarle dos veces si el autor del comentario también es el autor del post
